...
-- =====================================================
-- 3. 回调出站任务表 - 持久化发往律所系统的回调事件
-- =====================================================
CREATE TABLE IF NOT EXISTS public.callback_task (
    id BIGSERIAL PRIMARY KEY,
    matter_id VARCHAR(50),                         -- 项目ID
    callback_type VARCHAR(50) NOT NULL,           -- 回调类型
    callback_url VARCHAR(500) NOT NULL,           -- 回调目标地址
    callback_payload JSONB,                       -- 回调负载
    status VARCHAR(20) DEFAULT 'PENDING',         -- 状态：PENDING、SENDING、SENT、DEAD
    retry_count INTEGER DEFAULT 0,                -- 已重试次数
    max_retries INTEGER DEFAULT 12,               -- 最大重试次数
    next_retry_at TIMESTAMP,                      -- 下次重试时间
    last_attempt_at TIMESTAMP,                    -- 最后一次投递时间
...
CREATE INDEX IF NOT EXISTS idx_callback_task_status ON public.callback_task(status);
CREATE INDEX IF NOT EXISTS idx_callback_task_next_retry_at ON public.callback_task(next_retry_at);
CREATE INDEX IF NOT EXISTS idx_callback_task_matter_id ON public.callback_task(matter_id);

COMMENT ON TABLE public.callback_task IS '回调出站任务表 - 持久化发往律所系统的回调事件';
COMMENT ON COLUMN public.callback_task.callback_type IS '回调类型：ACCESS_LOG、DOWNLOAD_LOG、FILE_UPLOAD、NOTIFICATION_SUCCESS、NOTIFICATION_FAILURE';
COMMENT ON COLUMN public.callback_task.callback_payload IS '回调负载，JSON 格式';
COMMENT ON COLUMN public.callback_task.status IS '出站任务状态：PENDING、SENDING、SENT、DEAD';
COMMENT ON COLUMN public.callback_task.retry_count IS '已重试次数';
COMMENT ON COLUMN public.callback_task.max_retries IS '最大重试次数';
COMMENT ON COLUMN public.callback_task.next_retry_at IS '下次重试时间';
COMMENT ON COLUMN public.callback_task.last_attempt_at IS '最后一次投递时间';
COMMENT ON COLUMN public.callback_task.last_error IS '最后一次投递失败信息';
...
CREATE INDEX IF NOT EXISTS idx_letter_verification_code ON public.letter_verification(verification_code);
...
CREATE INDEX IF NOT EXISTS idx_letter_verification_application_no ON public.letter_verification(application_no);
CREATE INDEX IF NOT EXISTS idx_letter_verification_source_api_key_id ON public.letter_verification(source_api_key_id);
