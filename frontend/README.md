...
客户服务系统的前端应用，提供客户访问项目详情和文件管理的功能。
...
VITE_APP_NAME_EN=Professional Legal Service Portal  # 系统英文完整名称
VITE_APP_SHORT_NAME_EN=Professional Service         # 系统英文简称
VITE_PORTAL_TITLE=客户服务门户         # 客户服务门户标题
...
使用Pinia进行状态管理：
- `stores/apiKey.ts` - API密钥存储和管理

### API认证

- **客户门户API**：通过URL参数传递token（`?token=xxx`）
- **管理后台API**：通过Authorization header传递Bearer Token（`Authorization: Bearer ${apiKey}`）
  - API密钥存储在localStorage中
  - 首次访问管理后台需要先配置API密钥（访问 `/admin/settings`）
  - 路由守卫会自动检查API密钥，未配置时跳转到配置页面
